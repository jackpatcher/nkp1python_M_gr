...
class Motor:
    ...
    def motor(self, pin, _speed):
        ...
        pMinus =  1023 if abs(1023 - _speed)>1023   else abs(1023 - _speed)
        pPlus   =  1023  if  abs(1023 + _speed) >1023 else  abs(1023 + _speed)
        if pin == 1:
            if _speed > 0:
                self.m1a.duty(pMinus)
                self.m1b.duty(1023)
                sleep(0.001)
            elif _speed <= 0:
                self.m1a.duty(1023)
