...
def start_test_gyro():
    # ตั้งค่า I2C และ OLED
    i2c = I2C(0, scl=Pin(22), sda=Pin(21))
    oled = SSD1306_I2C(128, 64)
    
    oled.fill(0)
    oled.text("TEST GYRO",0, 0)
    oled.text("plz wait....",0, 10)
    oled.show()
    ...
    try:
        while True:
            ...
            print("มุม X: {:.2f}, มุม Y: {:.2f}, มุม Z: {:.2f}".format(
                -angles['x'], angles['y'], 2*angles['z']))

            # แสดงผลบน OLED
            oled.fill(0)
            oled.text("X: {:.2f}".format(-angles['x']), 0, 0)
            oled.text("Y: {:.2f}".format(angles['y']), 0, 10)
            oled.text("Z: {:.2f}".format(2*angles['z']), 0, 20)
            oled.show()
